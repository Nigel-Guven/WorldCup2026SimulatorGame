...
interface ThirdPlaceStandingsTableProps {
  groups: GroupState[];
  qualifierCutoff?: number; // e.g. 8 for World Cup 2026, 4 for Euros
}
...
export function ThirdPlaceStandingsTable({
  ...
  // Extract third-place teams from each group and sort cross-group
  ...
  if (thirdPlaceRankings.length === 0) return null;
      ...
      <table className="w-full text-[11px] text-left border-collapse">
        <thead>
          <tr className="bg-slate-950 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-800">
            ...
            <th className="py-2.5 px-2 text-center w-12">Grp</th>
            <th className="py-2.5 px-1 text-center w-10">P</th>
            <th className="py-2.5 px-1 text-center w-10">GD</th>
            <th className="py-2.5 px-2 text-right w-12">Pts</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-800/40 font-medium">
          {thirdPlaceRankings.map((row, idx) => {
            if (!row) return null;
            const isQualifying = idx < qualifierCutoff;
